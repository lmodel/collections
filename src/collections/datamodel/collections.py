...
# Generation date: 2023-03-11T20:32:51
# Schema: collections
#
...
class List(Bag):
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        self._normalize_inlined_as_dict(slot_name="item", slot_type=ListItem, key_name="index", keyed=False)
...
class ListItem(CoItem):
    ...
    index: Union[int, PositiveInteger] = None

    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        if self._is_empty(self.index):
            self.MissingRequiredField("index")
        if not isinstance(self.index, PositiveInteger):
            self.index = PositiveInteger(self.index)
...
slots.index = Slot(uri=COLLECTIONS.index, name="index", curie=COLLECTIONS.curie('index'),
                   model_uri=COLLECTIONS.index, domain=ListItem, range=Optional[Union[int, PositiveInteger]])
...
slots.ListItem_index = Slot(uri=COLLECTIONS.index, name="ListItem_index", curie=COLLECTIONS.curie('index'),
                   model_uri=COLLECTIONS.ListItem_index, domain=ListItem, range=Union[int, PositiveInteger])