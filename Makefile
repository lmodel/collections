...
GEN_PARGS = --no-mergeimports
ifdef LINKML_COOKIECUTTER_GEN_PROJECT_ARGS
GEN_PARGS = ${LINKML_COOKIECUTTER_GEN_PROJECT_ARGS}
endif

GEN_DARGS =--no-mergeimports
ifdef LINKML_COOKIECUTTER_GEN_DOC_ARGS
GEN_DARGS = ${LINKML_COOKIECUTTER_GEN_DOC_ARGS}
endif

GEN_OARGS = --no-mergeimports --no-type-objects --no-metaclasses --metadata-profile rdfs
ifdef LINKML_COOKIECUTTER_GEN_OWL_ARGS
GEN_OARGS = ${LINKML_COOKIECUTTER_GEN_OWL_ARGS}
endif

GEN_JARGS = --no-mergeimports --no-metadata
ifdef LINKML_COOKIECUTTER_GEN_JAVA_ARGS
GEN_JARGS = ${LINKML_COOKIECUTTER_GEN_JAVA_ARGS}
endif

GEN_TARGS = --no-mergeimports --no-metadata
ifdef LINKML_COOKIECUTTER_GEN_TYPESCRIPT_ARGS
GEN_TARGS = ${LINKML_COOKIECUTTER_GEN_TYPESCRIPT_ARGS}
endif
...
ifneq ($(strip ${GEN_OARGS}),)
	$(RUN) gen-owl ${GEN_OARGS} -o $(DEST)/owl/collections.owl.ttl $(SOURCE_SCHEMA_PATH)
endif
...
ifneq ($(strip ${GEN_TARGS}),)
	...
	$(RUN) gen-typescript ${GEN_TARGS} $(SOURCE_SCHEMA_PATH) >${DEST}/typescript/collections.ts
endif
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/collections/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py
